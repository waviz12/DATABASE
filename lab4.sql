...
-- Task 1.3: Projects with budget categories
SELECT
    project_name,
    budget,
    CASE
...
-- Task 1.4: Employee emails with COALESCE
SELECT
    first_name || ' ' || last_name AS full_name,
...
-- Task 2.4: Employees with manager and IT department
SELECT *
FROM employees
...
-- Task 3.1: Uppercase, length of last name, first 3 chars of email
SELECT
...
-- Task 3.2: Annual salary, monthly, 10% raise
SELECT
    first_name || ' ' || last_name AS full_name,
...
-- Task 4.2: Total hours worked on each project
SELECT
    p.project_name,
...
-- Task 4.3: Number of employees per department > 1
SELECT
    department,
...
-- Task 5.1: UNION - salary > 65000 or hired after 2020-01-01
...
-- Task 5.2: INTERSECT - IT dept AND salary > 65000
SELECT employee_id, first_name || ' ' || last_name AS full_name, salary
FROM employees
WHERE department = 'IT'
INTERSECT
...
-- Task 6.3: ANY - employees with salary > ANY in Sales
SELECT *
FROM employees
WHERE salary > ANY (
    SELECT salary
...
-- Task 7.1: Average hours per employee + rank by salary
SELECT
